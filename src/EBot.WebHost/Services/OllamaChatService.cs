        ...
        IReadOnlyDictionary<string, JsonElement> input, CancellationToken ct)
    {
        switch (name)
        {
            ...
            case "get_destinations":
            {
                ...
                try { if (File.Exists(path)) list = JsonSerializer.Deserialize<List<TravelDestination>>(File.ReadAllText(path), new JsonSerializerOptions { PropertyNameCaseInsensitive = true }) ?? []; } catch { }
